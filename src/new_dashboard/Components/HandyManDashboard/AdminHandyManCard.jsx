...
const AdminHandyManCard = () => {
    ...
    try {
      ...
      const apiCalls = [
        ...
        },
        {
          name: "totalEarnings",
          call: axios.post(
            `${serverEndPoint}/get_total_handyman_orders_and_earnings`,
            {},
            config
          ),
        },
        {
          ...
          call: axios.post(
            `${serverEndPoint}/get_handyman_current_month_earnings`,
            {},
            config
          ),
        },
        ...
        if (result.status === "fulfilled") {
          try {
            switch (apiCalls[index].name) {
              case "totalDrivers":
                // API returns total_count directly
                totalDrivers = parseInt(result.value.data.total_count || 0);
                break;
              case "waitingApproval":
                // API returns total_count directly
                waitingApproval = parseInt(result.value.data.total_count || 0);
                break;
              case "totalEarnings":
                // Ensure numeric value
                totalEarnings = parseFloat(
                  result.value.data.total_earnings || 0
                );
                break;
              case "monthlyEarnings":
                // Ensure numeric value
                monthlyEarnings = parseFloat(
                  result.value.data.current_month_earnings || 0
                );
                break;
            }
          } catch (error) {
            console.error(
              `Error processing ${apiCalls[index].name} data:`,
              error
            );
  ...
  const handleCardClick = () => {
    navigate("/dashboard/all-handyman");
  };
  const handleOrderClick = () => {
    navigate("/dashboard/handyman/orders");
  };
              ...
              <div className="card-body">
                ...
                <div className="d-flex justify-content-between align-items-center">
                  <div className="flex-shrink-0 align-self-end">
                    <p className="f-s-16 mb-0">HandyMan Agents</p>
                    <h5>{totalDrivers}</h5>
                  </div>
