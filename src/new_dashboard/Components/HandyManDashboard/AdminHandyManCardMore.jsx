...
const AdminHandyManCardMore = () => {
      ...
      const apiCalls = [
        {
          name: "todaysEarnings",
          call: axios.post(
            `${serverEndPoint}/get_handyman_today_earnings`,
            {},
            config
          ),
        },
        ...
        if (result.status === "fulfilled") {
          try {
            ...
            switch (apiCalls[index].name) {
              case "todaysEarnings":
                // Safely parse float or use default
                todayEarn = parseFloat(data.today_earnings || 0.0) || 0.0;
                break;
              case "totalRides":
                // Safely parse float or use default
                totalRides = parseFloat(data.total_orders || 0.0) || 0.0;
                break;
            }
          } catch (error) {
            console.error(
  ...
  const handleCardClick = () => {
    navigate("/dashboard/all-handyman-orders-report");
  };
