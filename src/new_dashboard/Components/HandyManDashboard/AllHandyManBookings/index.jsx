...
const AllHandyManBookings = () => {
  ...
  const filterBookings = (bookings, query) => {
    return bookings.filter((order) => {
      return (
        ...
        order.handyman_name.toLowerCase().includes(query) ||
  ...
  const fetchAllBookingsData = async () => {
    ...
    try {
      setLoading(true);
      const response = await axios.post(
        `${serverEndPoint}/get_handyman_all_ongoing_bookings_details`,
        { allBookings: 1 },
        config
      );
  ...
  const fetchAllCancelledBookingsData = async () => {
    ...
    try {
      setLoading(true);
      const response = await axios.post(
        `${serverEndPoint}/get_handyman_all_cancelled_bookings_details`,
        {},
        config
      );
  ...
  const renderBookingTable = (
    ...
    <div className="order-list-table table-responsive app-scroll">
      <input
        ...
        placeholder="Search by Booking ID, Customer, Handyman, or Mobile"
        value={searchQuery}
            ...
            <tr key={index}>
              ...
              <td>
                <div className="position-relative">
                  ...
                  <div className="ms-5">
                    <h6 className="mb-0 f-s-16">{order.handyman_name}</h6>
                    <p className="mb-0 f-s-14 text-secondary">
              ...
              <td>
                <Link
                  to={`/dashboard/handyman-booking-details/${order.booking_id}`}
                  target="_blank"
  ...
  return (
    <div>
      <Container fluid>
        <Row className="m-1">
          <Col xs={12}>
            <h4 className="main-title">Handyman Bookings</h4>
...
export default AllHandyManBookings;
