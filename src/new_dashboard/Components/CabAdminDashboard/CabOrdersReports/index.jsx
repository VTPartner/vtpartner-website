...
const CabOrdersReport = () => {
  ...
  const fetchReport = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${serverEndPoint}/get_cab_orders_report`,
        {
  ...
  const exportToExcel = () => {
    ...
    const exportData = reportData.results.map((order) => ({
      ...
      "Vehicle Plate No": order.vehicle_plate_no,
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Cab Orders Report");

    // Save the file
    XLSX.writeFile(
      wb,
      `Cab_Orders_Report_${format(range[0], "yyyy-MM-dd")}_to_${format(
        range[1],
        "yyyy-MM-dd"
      )}.xlsx`
    );
  ...
  return (
    <Container fluid>
      <Row className="m-1">
        <Col xs={12}>
          <h4 className="main-title">Cab Orders Report</h4>
...
export default CabOrdersReport;
