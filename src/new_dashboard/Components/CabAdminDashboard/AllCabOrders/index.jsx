  ...
  const fetchAllOrdersData = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${serverEndPoint}/get_cab_all_completed_orders_details`,
        { key: 1 },
        config
      );
  ...
  return (
    <div>
      <Container fluid>
        <Row className="m-1">
          <Col xs={12}>
            <h4 className="main-title">Cab Orders</h4>
                        ...
                        <tbody>
                          {currentItems.map((order, index) => (
                            <tr key={index}>
                              ...
                              <td>
                                <Link
                                  to={`/dashboard/cab-order-details/${order.booking_id}/${order.order_id}`}
                                  target="_blank"
