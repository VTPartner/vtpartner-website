...
const JcbCraneOrdersReport = () => {
  ...
  const fetchReport = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${serverEndPoint}/get_jcb_crane_orders_report`,
        {
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "JCB Crane Orders Report");

    // Save the file
    XLSX.writeFile(
      wb,
      `JCB_Crane_Orders_Report_${format(range[0], "yyyy-MM-dd")}_to_${format(
        range[1],
        "yyyy-MM-dd"
      )}.xlsx`
    );
  ...
  return (
    <Container fluid>
      <Row className="m-1">
        <Col xs={12}>
          <h4 className="main-title">JCB/Crane Orders Report</h4>
...
export default JcbCraneOrdersReport;
