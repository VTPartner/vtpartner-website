...
const AllJcbCraneOrders = () => {
  ...
  const fetchAllOrdersData = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${serverEndPoint}/get_jcb_crane_driver_all_completed_orders_details`,
        { key: 1 },
        config
      );
  ...
  const filteredOrders = allOrders.filter((order) => {
    return (
      ...
      order.driver_name.toLowerCase().includes(searchQuery) ||
  ...
  return (
    <div>
      <Container fluid>
        <Row className="m-1">
          <Col xs={12}>
            <h4 className="main-title">JCB Crane Orders</h4>
            <ul className="app-line-breadcrumbs mb-3">
              <li className="">
                <a href="#" className="f-s-14 f-w-500">
                  <span>
                    <i className="ph-duotone ph-crane f-s-16"></i> JCB Crane
                  </span>
                </a>
              </li>
                        ...
                        <tbody>
                          {currentItems.map((order, index) => (
                            <tr key={index}>
                              ...
                              <td>
                                <Link
                                  to={`/dashboard/jcb-crane-order-details/${order.booking_id}/${order.order_id}`}
                                  target="_blank"
...
export default AllJcbCraneOrders;
