...
const AgentsCancelReasons = () => {
  ...
  const fetchReasons = async () => {
    ...
    try {
      setLoading(true);
      const response = await axios.post(
        `${serverEndPoint}/get_driver_cancel_reasons`,
        {
      ...
      toast.error("Failed to fetch driver cancel reasons");
    } finally {
      setLoading(false);
    }
  };
  ...
  const saveReasonDetails = async () => {
    ...
    try {
      ...
      const endpoint = isEditMode
        ? `${serverEndPoint}/update_driver_cancel_reason`
        : `${serverEndPoint}/add_driver_cancel_reason`;
      ...
      if (response.status === 200) {
        toast.success(
          isEditMode
            ? "Driver reason updated successfully!"
            : "Driver reason added successfully!"
        );
        fetchReasons();
        setOpenDialog(false);
      }
...
export default AgentsCancelReasons;
