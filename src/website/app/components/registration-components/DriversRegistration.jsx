...
const DriversRegistrationHeroBanner = () => {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      if (response.status === 200) {
        toast.success("Drivers Registration successful!");
        resetForm();
      }
    } catch (error) {
      handleError(error);
  ...
  const handleSendVerificationCode = async (e) => {
    ...
    setBtnLoading(true);
    try {
      //   const formattedPhoneNumber = formatPhoneNumber(mobileNumber, "91"); // Replace 'IN' with your country code
      ...
      // Handle the confirmation result, e.g., send SMS verification code
      ...
      toast.error("Error Sending the Verification code ");
    } finally {
  ...
  const handleVerifyCode = async () => {
    try {
      const confirmationResult = await confirmationResult.confirm(
        verificationCode
      );
      // User is signed in
  ...
  function onSignUp(e) {
    ...
    const formatPhoneNo = "+91" + mobileNumber;
    console.log("formatPhoneNo::", formatPhoneNo);
    signInWithPhoneNumber(auth, formatPhoneNo, appVerifier)
      .then((confirmationResult) => {
        // SMS sent. Prompt user to type the code from the message, then sign the
        // user in with confirmationResult.confirm(code).
        window.confirmationResult = confirmationResult;
        setBtnLoading(false);
        //show verify now button and hide send otp button
        toast.success("OTP Sent successfully");
      })
      .catch((error) => {
        // Error; SMS not sent
        ...
        setBtnLoading(false);
        toast.error("Error sending OTP please try after sometime");
      });
  }
          ...
          <Box className="bg-white p-8 shadow-lg rounded-sm w-full sm:max-w-[30rem] max-w-[40rem] max-h-[38rem]">
            ...
            <form className="pb-4">
              ...
              >
                ...
                >
                  <MenuItem value="customer_referral">
                    Customer Referral
                  </MenuItem>
              ...
              <div id="recaptcha-container" className="text-red-500"></div>
              <button
                onClick={onSignUp}
                className="bg-emerald-600 w-full flex gap-1 items-center justify-center py-2.5 text-white rounded-sm mt-2"
              >
                {btnLoading && <CgSpinner size={20} className="animate-spin" />}
                <span>Send OTP</span>
              </button>
...
export default DriversRegistrationHeroBanner;
