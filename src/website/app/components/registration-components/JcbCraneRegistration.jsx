...
const JcbCraneRegistrationHeroBanner = () => {
  ...
  const handleError = (error) => {
    if (error.response) {
      if (error.response.status === 404) {
        ...
        toast.error(
          "This Service JcbCraneRegistrationHeroBanner has already been sent to us.\nWe Are working on it"
        );
        setError("Internal Server Error");
      } else {
  ...
  const handleServiceChange = async (event) => {
    ...
    const selectedService = services.find(
      (service) => service.category_id === event.target.value
    );
    console.log("selectedService:", selectedService);
    // Update selected service in state
    setSelectedService({
      category_id: selectedService.category_id,
      category_type: selectedService.category_type,
    });

    // Trigger async actions based on category_type
    if (selectedService.category_type === "Delivery") {
      console.log(selectedService.category_id);
      await fetchAllVehicles(selectedService.category_id);
    } else if (selectedService.category_type === "Service") {
      await fetchSubCategory(selectedService.category_id);
    }
  };
  ...
  const validateForm = () => {
    ...
    if (selectedService.category_id === -1)
      newErrors.service = "Please select a service.";

    // Conditionally check for subcategories or vehicles
    if (selectedService.category_type === "Service") {
  ...
  const resetForm = () => {
    ...
    setSelectedService({ category_id: -1, category_type: "" }); // Reset selected service
    setSelectedSubcategory({ sub_cat_id: -1, sub_cat_name: "" }); // Reset subcategory
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      if (response.status === 200) {
        toast.success("JcbCraneRegistrationHeroBanner successful!");
        resetForm();
      }
    } catch (error) {
      handleError(error);
  ...
  return (
    <div className="relative w-full sm:h-full sm:mt-[3rem] mt-[4rem] h-full sm:p-12 p-2  bg-white">
      ...
      {/* JcbCraneRegistrationHeroBanner Form Card */}
      <div className="relative z-10 flex items-center justify-end h-full w-full sm:pr-10">
        <Box className="bg-white p-8 shadow-lg rounded-sm w-full sm:max-w-[30rem] max-w-[40rem] max-h-[38rem]">
          <h2 className="sm:text-2xl font-titillium mb-4">
            Attach your JCB or Crane Vehicle
          </h2>
          ...
          <form className="pb-4">
            ...
            {/* <div className="flex mb-4 mt-3">
              ...
              >
                <InputLabel id="service-type-label">Register As?</InputLabel>
                <Select
                  labelId="service-type-label"
                  id="service-type"
                  label="Service Type"
                  value={selectedService.category_id} // Track by category_id
                  onChange={handleServiceChange} // Update the whole service object
                >
            ...
            {selectedService.category_type === "Service" &&
...
export default JcbCraneRegistrationHeroBanner;
