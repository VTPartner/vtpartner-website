...
const DriversEstimationForm = ({ onCitySelect }) => {
  ...
  const validationSchema = Yup.object().shape({
    ...
    requiredHours: Yup.number()
      ...
      .max(24, "Maximum of 24 hour Only You can increase the days"), // Add a minimum if necessary
    requiredDays: Yup.number()
      .typeError("Approx Days must be a number")
      .required("Approx Days is required!")
      .max(24, "Maximum of 24 Days Only")
      .min(1, "Minimum of 1 Days is required"), // Add a minimum if necessary
    contactName: Yup.string().required("Full Name is required!"),
    contactNumber: Yup.string()
      .required("Phone Number is required!")
      .matches(/^[0-9]+$/, "Phone number must contain only digits") // Allow only digits
            ...
            {({ values, errors, touched, handleBlur, handleSubmit }) => (
              <form onSubmit={handleSubmit}>
                ...
                <TextField
                  ...
                  label="Approx Days"
                  autoComplete="off"
                  placeholder="Enter approx days to need the driver for [Ex : 1]"
                  type="tel"
                  margin="dense"
                  size={isMobile ? "small" : "medium"}
                  value={requiredDays}
                  onChange={(e) => {
                    ...
                    values.requiredDays = days; // Set the new value directly
                  }}
                  variant="outlined"
                  className="mb-4"
                  InputProps={{
                  ...
                  onBlur={handleBlur}
                  helperText={touched.requiredDays && errors.requiredDays}
                  error={Boolean(errors.requiredDays && touched.requiredDays)}
                />
...
export default DriversEstimationForm;
